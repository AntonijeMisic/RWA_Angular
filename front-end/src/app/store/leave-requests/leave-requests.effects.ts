...
export class LeaveRequestEffects {
    ...
    private leaveRequestService: LeaveRequestService = inject(LeaveRequestService);
    ...
    loadLeaveRequests$ = createEffect(() =>
        ...
        exhaustMap(() =>
            ...
            map(leaveRequests => LeaveRequestActions.loadLeaveRequestsSuccess({ leaveRequests })),
            catchError(error => of(LeaveRequestActions.loadLeaveRequestsFailure({ error })))
            )
        )
        )
    );
    ...
    loadUserLeaveRequests$ = createEffect(() =>
        ...
        exhaustMap(action =>
            ...
            map(leaveRequestsByUser => LeaveRequestActions.loadUserLeaveRequestsSuccess({ leaveRequestsByUser })),
            catchError(error => of(LeaveRequestActions.loadUserLeaveRequestsFailure({ error })))
            )
        )
        )
    );
    ...
    loadApprovedLeavesForWeek$ = createEffect(() =>
        this.actions$.pipe(
            ...
            this.leaveRequestService.getApprovedLeavesForWeek(userId, start, end).pipe(
                map(leaves => LeaveRequestActions.loadApprovedLeavesForWeekSuccess({ leaves })),
                catchError(err =>
                of(LeaveRequestActions.loadApprovedLeavesForWeekFailure({ error: err.message }))
                )
            )
        )
        )
    ...
    createLeaveRequest$ = createEffect(() =>
        ...
        exhaustMap(action =>
            ...
            map(leaveRequest => LeaveRequestActions.createLeaveRequestSuccess({ leaveRequest })),
            catchError(error => of(LeaveRequestActions.createLeaveRequestFailure({ error })))
            )
        )
        )
    );

    deleteLeaveRequest$ = createEffect(() => //ne znam da li ce da mi treba
        this.actions$.pipe(
        ofType(LeaveRequestActions.deleteLeaveRequest),
        exhaustMap(action =>
            this.leaveRequestService.deleteRequest(action.requestId).pipe(
            map(() => LeaveRequestActions.deleteLeaveRequestSuccess({ requestId: action.requestId })),
    ...
    updateLeaveRequestStatus$ = createEffect(() =>
        this.actions$.pipe(
            ...
            this.leaveRequestService.updateStatus(action.dto).pipe(
                map((leaveRequest) =>
                    LeaveRequestActions.updateLeaveRequestStatusSuccess({ leaveRequest })
                    ),
                catchError((error) =>
